...
현재 워커는 native tool / OpenTerminal 요청을 `stream=true`로 보내고 SSE를 끝까지 읽습니다. 따라서 이 경로에서 실패하면:

- upstream 모델 서버의 SSE 형식이 OpenAI 호환인지
- 프록시가 `text/event-stream`을 buffer 없이 통과시키는지
- Open WebUI가 final assistant text를 실제로 스트림에 내보내는지
