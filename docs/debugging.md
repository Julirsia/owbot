...
3. 모델 completion / tool / terminal / skill 실행 문제
...
- 워커는 `terminal_id`만 Open WebUI에 보냅니다.
- 실제 OpenTerminal API 호출은 Open WebUI가 terminal connection `url`을 통해 수행합니다.
- 그래서 terminal 디버깅은 `워커 -> Open WebUI`와 `Open WebUI -> OpenTerminal` 두 구간으로 나눠 봐야 합니다.
...
가능하면 OpenTerminal 자체 스펙도 같이 확인합니다.

```bash
curl -sS "$OPENWEBUI_BASE_URL/api/v1/terminals/$OPENWEBUI_TERMINAL_ID/openapi.json" \
  -H "Authorization: Bearer $OPENWEBUI_BOT_TOKEN"
```
...
## 10-1. skill 호출이 안 될 때

skill은 두 층을 동시에 봐야 합니다.

1. Open WebUI model preset에 skill이 연결되어 있는지
2. skill 내용이 참조하는 filesystem/script가 OpenTerminal 쪽에 실제 존재하는지

특히 filesystem install 기반 skill이면:

- Open WebUI native skill 메타가 있어야 하고
- OpenTerminal 작업 디렉터리 아래 skill 폴더와 스크립트도 실제로 있어야 합니다

예를 들어 아래를 통해 확인할 수 있습니다.

```bash
curl -sS "$OPENWEBUI_BASE_URL/api/v1/terminals/$OPENWEBUI_TERMINAL_ID/files/cwd" \
  -H "Authorization: Bearer $OPENWEBUI_BOT_TOKEN"
```

그리고 필요하면 skill 디렉터리를 직접 확인합니다.

```bash
curl -sS "$OPENWEBUI_BASE_URL/api/v1/terminals/$OPENWEBUI_TERMINAL_ID/files/list?directory=/path/to/SKILL" \
  -H "Authorization: Bearer $OPENWEBUI_BOT_TOKEN"
```

실패 패턴:

- skill 이름은 보이는데 스크립트 실행 결과가 없음
  - skill prompt만 쓰고 terminal script는 못 탄 상태
- Open WebUI UI에서는 되는데 worker에서는 안 됨
  - worker 요청에 `terminal_id` / `skill_ids` / native mode가 기대대로 실리지 않는지 확인
- worker와 UI 모두 안 됨
  - skill 설치 또는 OpenTerminal filesystem export 자체를 먼저 의심
