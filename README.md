...
기본적으로 모델 UI에 저장된 도구 설정을 따르며, `OPENWEBUI_TOOL_IDS` / `OPENWEBUI_TOOL_SERVER_IDS` / `OPENWEBUI_FEATURES_JSON`를 지정한 경우에만 해당 값으로 명시 오버라이드합니다. `OPENWEBUI_TERMINAL_ID` / `OPENWEBUI_SKILL_IDS`는 별도로 추가 전달되며, tool/terminal 경로도 Open WebUI의 `/api/chat/completions` 단일 응답을 그대로 사용합니다.
